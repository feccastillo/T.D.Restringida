...
/**
 * Implementasion de algoritmo canny
 * 
 * @author Fecc
 * 
 */
public class CannyAlgoritmo extends Component {
	private static final long serialVersionUID = 1L;

	// escala 8 bits de magnitud y dirección
	...
	// procesa la imagen
	public void process(Image img) throws Exception {
		sourceImage=img;
	...
	private void canny(float f, int gkernel) {
		...
		do {
			...
			// media gaussiana
			meanGauss[k4] = gauss(k4, f) + gauss((float) k4 - 0.5F, f) + gauss((float) k4 + 0.5F, f);
			meanGauss[k4] = meanGauss[k4] / 3F / (6.283185F * f * f);
			af5[k4] = gauss((float) k4 + 0.5F, f) - gauss((float) k4 - 0.5F, f);
			k4++;
		} while (true);

		// pasa a lo largo de x e y con la Gaussiana
		int j = k4;
		j1 = width - (j - 1);
		int l = width * (j - 1);
		...
		for (int l4 = j - 1; l4 < j1; l4++) {
			for (int l5 = l; l5 < i1; l5 += width) {
				...
				// pasa por x con la gaussiana
				convy[k1] = tmp1;
				// pasa con y con la gaussiana
				convx[k1] = tmp2;
			}
		}
		...
		for (int k5 = j; k5 < j1; k5++) {
			for (int k6 = l; k6 < i1; k6 += width) {
				...
				// se y*x <= 0 (estamos en el segundo o cuarto cuadrante)
				if (tmp1 * tmp2 <= 0) {
					// se y >= x
					if (Math.abs(tmp1) >= Math.abs(tmp2)) {

						if (Math.abs(tmp1 * f12) >= Math.abs(tmp2 * tmp8 - (tmp1 + tmp2) * tmp6)
								&& Math.abs(tmp1 * f12) > Math.abs(tmp2 * tmp9 - (tmp1 + tmp2) * tmp5)) {
							vabene = true;
						} else
							vabene = false;
					} else if (Math.abs(tmp2 * f12) >= Math.abs(tmp1 * tmp8 - (tmp2 + tmp1) * tmp3)
							&& Math.abs(tmp2 * f12) > Math.abs(tmp1 * tmp9 - (tmp2 + tmp1) * tmp4)) {
						vabene = true;
					} else
						vabene = false;
				} else {
					if (Math.abs(tmp1) >= Math.abs(tmp2)) {
						if (Math.abs(tmp1 * f12) >= Math.abs(tmp2 * tmp10 + (tmp1 - tmp2) * tmp6)
								&& Math.abs(tmp1 * f12) > Math.abs(tmp2 * tmp7 + (tmp1 - tmp2) * tmp5)) {
							vabene = true;
						} else
							vabene = false;
					} else if (Math.abs(tmp2 * f12) >= Math.abs(tmp1 * tmp10 + (tmp2 - tmp1) * tmp4)
							&& Math.abs(tmp2 * f12) > Math.abs(tmp1 * tmp7 + (tmp2 - tmp1) * tmp3)) {
						vabene = true;
					} else
						vabene = false;
				}
	...
	// devuelve 0 si son ambos cero
	// de lo contrario, devuelve el módulo
	...
	private void thresholding(int i, int j) {
		if (i < j) {
			System.out.println("Errore: umbral superior < umbral inferior!");
		} else {
	...
	private Image pixels2image(int ai[]) {
		MemoryImageSource memoryimagesource = new MemoryImageSource(width, height, ColorModel.getRGBdefault(), ai, 0,
				width);
	...
	public static BufferedImage toBufferedImage(Image img)
	{
	    ...
	    // Crear una imagen almacenada con transparencia
	    BufferedImage bimage = new BufferedImage(img.getWidth(null), img.getHeight(null), BufferedImage.TYPE_INT_ARGB);

	    // Dibuje la imagen en la imagen almacenada
	...
	public File getEdgeImageFile() {

		BufferedImage bi = toBufferedImage(edgeImage);
